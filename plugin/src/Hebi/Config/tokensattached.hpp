...
class Tokens : public QQuickAttachedPropertyPropagator, public QQmlParserStatus {
    ...
    Q_PROPERTY(const hebi::config::AppearanceRounding* rounding READ rounding NOTIFY sourceChanged)
    Q_PROPERTY(const hebi::config::AppearanceSpacing* spacing READ spacing NOTIFY sourceChanged)
    Q_PROPERTY(const hebi::config::AppearancePadding* padding READ padding NOTIFY sourceChanged)
    Q_PROPERTY(const hebi::config::AppearanceTransparency* transparency READ transparency NOTIFY sourceChanged)
    Q_PROPERTY(const hebi::config::SizeTokens* sizes READ sizes NOTIFY sourceChanged)
    Q_PROPERTY(const hebi::config::FontTokens* font READ font NOTIFY sourceChanged)
    Q_PROPERTY(const hebi::config::AnimTokens* anim READ anim NOTIFY sourceChanged)
